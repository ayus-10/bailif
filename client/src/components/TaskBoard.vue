        ...
        <div class="d-flex align-center mb-4">
            <div>
                ...
                <span class="text-body-2 text-medium-emphasis">
                    ...
                    <span v-if="route.query.status || route.query.priority">matching filters</span>
                </span>
            </div>
            <v-spacer />
            <v-btn color="primary" prepend-icon="mdi-plus" variant="flat">New Task</v-btn>
        </div>
        ...
        <v-row>
            <v-col v-for="column in columns" :key="column.status" cols="12" md="4">
                <div class="d-flex align-center mb-2">
                    ...
                    <span class="text-subtitle-2 font-weight-medium">{{ column.label }}</span>
                    <v-chip size="x-small" class="ml-2">{{ tasksFor(column.status).length }}</v-chip>
                </div>
...
const tasks = [
    { id: 1, title: "Finalize homepage wireframes", project: "Website Redesign", due: "Today", priority: "high", status: "todo" },
    { id: 2, title: "Review API contract", project: "Mobile App", due: "Tomorrow", priority: "medium", status: "todo" },
    { id: 3, title: "Approve campaign copy", project: "Marketing Campaign", due: "Jul 28", priority: "low", status: "todo" },
    { id: 4, title: "Build component library", project: "Website Redesign", due: "Jul 30", priority: "medium", status: "in-progress" },
    { id: 5, title: "Wire up auth flow", project: "Mobile App", due: "Aug 1", priority: "high", status: "in-progress" },
    { id: 6, title: "Set up staging environment", project: "Mobile App", due: "Jul 29", priority: "medium", status: "done" },
    { id: 7, title: "QA pass on staging", project: "Website Redesign", due: "Aug 2", priority: "low", status: "done" },
];

const filteredTasks = computed(() =>
    tasks.filter((task) => {
        if (route.query.status && task.status !== route.query.status) return false;
        if (route.query.priority && task.priority !== route.query.priority) return false;
        return true;
    })
);

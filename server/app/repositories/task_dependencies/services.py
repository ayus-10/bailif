...
def create_dependency(db: Session, payload: TaskDependencyCreate) -> TaskDependency:
    task_exists = db.get(Task, payload.task_id)
    depends_on_exists = db.get(Task, payload.depends_on_id)
    if task_exists is None or depends_on_exists is None:
        raise ReferencedTaskNotFoundError(
            f"{payload.task_id} or {payload.depends_on_id}"
        )

    existing = db.execute(
        select(TaskDependency).where(
            TaskDependency.task_id == payload.task_id,
            TaskDependency.depends_on_id == payload.depends_on_id,
            TaskDependency.dependency_type == payload.dependency_type,
        )
    ).scalar_one_or_none()
    if existing is not None:
        raise DuplicateDependencyError(f"{payload.task_id}->{payload.depends_on_id}")

    edge = _normalized_edge(
        payload.task_id, payload.depends_on_id, payload.dependency_type
    )
    if edge is not None and _would_create_cycle(db, edge[0], edge[1]):
        raise CycleDetectedError(f"{payload.task_id}->{payload.depends_on_id}")
